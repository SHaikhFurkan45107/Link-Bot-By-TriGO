...
// Initialize a Map to track the number of links generated by each user
let userUsageMap = new Map();

bot.on('callback_query', (callbackQuery) => {
    const userId = callbackQuery.from.id;  // Get the user ID from the callback query

    // Check if the user has already generated links
    if (!userUsageMap.has(userId)) {
        userUsageMap.set(userId, 0);  // Initialize the count to 0 for new users
    }

    // Get the current count of links generated by the user
    let userUsageCount = userUsageMap.get(userId);

    // Increment the count
    userUsageCount++;

    // Update the map with the new count
    userUsageMap.set(userId, userUsageCount);

    // Now you can use userUsageCount to enforce your limits or other logic
    if (userUsageCount > 7) {
        bot.sendMessage(userId, "You've reached the limit of 7 links per day.");
    } else {
        // Generate the link and send it to the user
        bot.sendMessage(userId, `You have generated ${userUsageCount} links today.`);
    }
});
